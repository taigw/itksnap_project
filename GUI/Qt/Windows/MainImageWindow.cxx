...
void MainImageWindow::UpdateWindowTitle()
{
  ...
  if(projfile.length())
    {
    this->setWindowTitle(QString("%1 - U-Seg").arg(projfile));
    }
  else if(mainfile.length() && segfile.length())
    {
    this->setWindowTitle(QString("%1 - %2 - U-Seg").arg(mainfile).arg(segfile));
    }
  else if(mainfile.length())
    {
    this->setWindowTitle(QString("%1 - New Segmentation - U-Seg").arg(mainfile));
    }
  else
    {
    this->setWindowTitle("U-Seg");
